...
#pragma omp parallel for
      ...
      if(false){
        //gpu_mcl_data.d_bn_exponents.copy_to_host(gpu_mcl_data.h_bn_exponents, stream);
        //gpu::sync(stream);
        //std::vector<char> density_tmp(density.size());
        //gpu::copy_gpu_to_cpu(density_tmp.data(), gpu_mcl_data.d_density.ptr, density.size(), stream);
        //gpu::sync(stream);
        //int cmp = memcmp(gpu_mcl_data.h_bn_exponents.ptr, bn_exponents.data(), 32*bn_exponents.size());
        //assert(cmp == 0);
        //for(int i = 0; i < density.size(); i++){
        //    int a = (int)density[i];
        //    int b = (int)density_tmp[i];
        //    assert(a == b);
        //}
       // assert(acc == gpu_acc);
      }
...
#pragma omp parallel for
      ...
      //gpu::sync(stream);
      //for(int i = 0; i < bn_exponents.size(); i++){
      //  memcpy(bn_exponents[i].data, gpu_mcl_data.h_bn_exponents.ptr + i, 32);
      //}
      // gpu::copy_gpu_to_cpu(h_density.data(), gpu_mcl_data.d_density.ptr, density.size(), stream);
      //gpu::sync(stream);
      //for(int i = 0 ;i < density.size(); i++){
      //  density[i] = (h_density[i] == 1);
      //}
