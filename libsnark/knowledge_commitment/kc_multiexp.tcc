    ...
    if(true){
      ...
      if(gpu_bucket_counters == nullptr){
        ...
        for(int i = 0; i < chunks; i++){
          ...
          d_buckets[i].init((1<<c) * instances);
          d_buckets2[i].init((1<<c));
          d_block_sums[i].init((1<<c) / 32);
          d_block_sums2[i].init((1<<c) / 32/32);
        }
      }
      if(gpu_bucket_counters == nullptr){
